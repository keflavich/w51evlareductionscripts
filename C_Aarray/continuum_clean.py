...
5/29/2014: deeper clean over a huge area
"""
...
mshigh = ['cont_spw{0:02d}'.format(int(spw)) for spw in high.split(",")]
mslow = ['cont_spw{0:02d}'.format(int(spw)) for spw in low.split(",")]
msboth = mslow+mshigh

for vis in msboth:
    listobs(vis=vis)

concat(mshigh,'cont_merge_high.ms')
concat(mslow,'cont_merge_low.ms')
mshigh,mslow = 'cont_merge_high.ms','cont_merge_low.ms'

myclean('','4096_low_uniform' ,'clean',weighting='uniform',imsize=[4096,4096],cell=['0.1 arcsec'],vis=mslow)
myclean('','4096_high_uniform','clean',weighting='uniform',imsize=[4096,4096],cell=['0.1 arcsec'],vis=mshigh)
myclean('','4096_both_uniform','clean',weighting='uniform',imsize=[4096,4096],cell=['0.1 arcsec'],vis=msboth)
