...
plotms(vis=vis, spw=str(spwn), xaxis='channel', yaxis='amp', avgtime='1e8',
        ...
        plotfile='' if INTERACTIVE else 'ampvschannel_spw%i.png' % (spwn),
        field=field,
        overwrite=True,
        )
...
for ant in antnames:
    plotms(vis=vis, spw=str(spwn), xaxis='time', yaxis='amp', avgchannel='128',
            ...
            plotfile='ampvstime_spw%i_ant%s.png' % (spwn,ant),
            field=field,
            overwrite=True,
            )
    ...
    plotms(vis=vis, spw=str(spwn), xaxis='freq', yaxis='phase', avgtime='1e8',
            ...
            plotfile='phasevsfreq_spw%i_ant%s.png' % (spwn,ant),
            field=field,
            overwrite=True,
            )
...
for calnum in xrange(10):
    ...
    if doplots:
        ...
        plotcal(caltable=caltable,
                ...
                showgui=INTERACTIVE,
                overwrite=True,
                figfile='' if INTERACTIVE else 'selfcal%i_spw%i_ampvstime.png' % (calnum,spwn),
                iteration='spw' if INTERACTIVE else '')#, subplot = 221)
        ...
        plotcal(caltable=caltable,
                ...
                showgui=INTERACTIVE,
                overwrite=True,
                figfile='' if INTERACTIVE else 'selfcal%i_spw%i_ampvsphase.png' % (calnum,spwn),
                iteration='spw' if INTERACTIVE else '')#, subplot = 221)
        ...
        plotms(vis=avg_data,
                ...
                overwrite=True,
                title='Iteration %i for spw %i' % (calnum,spw),
                plotfile='' if INTERACTIVE else 'selfcal%i_spw%i_amptime.png' % (calnum,spwn),
                )
    ...
    viewer(selfcal_image+".image",
            outfile=selfcal_image+".image.png",
            outformat='png',
            gui=False)
